...
export const settingsMenuRowLabelClass =
  "text-base font-normal text-text-primary opacity-90 transition-opacity duration-[var(--duration-ui-medium)] ease-[var(--ease-ui)] group-hover:opacity-100 max-md:group-active:opacity-100";

/** Button row chevron — visible on hover / mobile active. */
export const settingsMenuRowChevronClass =
  "pointer-events-none absolute top-1/2 right-4 shrink-0 -translate-y-1/2 opacity-0 transition-opacity duration-[var(--duration-ui-medium)] ease-[var(--ease-ui)] group-hover:opacity-40 max-md:group-active:opacity-40";
